...
public class ChatsService {
    ...
    public void addChats(ChatsDTO.RequestDTO chatsDTO, Integer usersId, MultipartFile multipartFile) throws IOException {
...
//        chatsRepository.save(chatsDTO.toEntity(user));
    ...
    public void deleteChats(Long chatsId, Integer usersId, ChatsDTO.RequestDTO chatsDTO) {
        User user = userRepository.findById(usersId).orElseThrow(() -> new IllegalArgumentException("해당 유저가 존재하지 않습니다."));
        chatsDTO.toEntity(user).deleteChats(chatsDTO.getDeleted());
//        s3UploadService.deleteImage();
    }

    public void updateChats(Long chatsId, ChatsDTO.RequestDTO chatsDTO, Integer usersId) {
